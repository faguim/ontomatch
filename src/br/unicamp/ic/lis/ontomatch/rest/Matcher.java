...
public class Matcher {
	...
	// + "SELECT DISTINCT ?resource ?label (f:LevenshteinFilter(?label, \"" + text +
	// "\") as ?similarity)"
	// + "WHERE { ");
	//
	// queryString.append("{?resource rdfs:label ?label . FILTER
	// (f:LevenshteinFilter(?label, \"" + text + "\") >= "
	// + similarity + ") }");
	//
	// queryString.append("UNION {?resource oboinowl:hasExactSynonym ?label . FILTER
	// (f:LevenshteinFilter(?label, \""
	// + text + "\") >= " + similarity + ")} ");
	// queryString.append("UNION {?resource oboinowl:hasRelatedSynonym ?label .
	// FILTER (f:LevenshteinFilter(?label, \""
	// + text + "\") >= " + similarity + ")} ");
	// queryString.append("} ORDER BY DESC(f:LevenshteinFilter(?label, \"" + text +
	// "\")) LIMIT 1");
	//
	// Query sparql = QueryFactory.create(queryString.toString());
	// QueryExecution qExec = QueryExecutionFactory.create(sparql,
	// getModel(ontology));
...
//		query.append("PREFIX ontomatch: <java:br.unicamp.ic.lis.ontomatch.filters.> \n");
//
//		query.append("SELECT DISTINCT ?resource ?label ?upperClass ?value (ontomatch:" + algorithm + "Filter(?label, \""
//				+ text + "\") as ?similarity) \n");
//		query.append("WHERE{ \n");
//		query.append("                 { ?resource        rdfs:label      ?label                 .   }   \n");
//		query.append("            UNION                                                                  \n");
				...
				for (int j = 0; j < utterances.length(); j++) {
					...
					for (int k = 0; k < phrases.length(); k++) {
						...
						for (int l = 0; l < mappings.length(); l++) {
							JSONArray mappingCandidates =  (JSONArray) ( (JSONObject)mappings.get(l)).get("MappingCandidates");
							for (int m = 0; m < mappingCandidates.length(); m++) {
								Resource resource = new Resource();
								resource.setLabel( ((JSONObject)mappingCandidates.get(m)).get("CandidatePreferred").toString() );
								resource.setUri(((JSONObject)mappingCandidates.get(m)).get("CandidateCUI").toString());

								JSONArray matchedWords = (JSONArray) ((JSONObject)mappingCandidates.get(m)).get("MatchedWords");
